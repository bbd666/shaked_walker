...
ControlAlgorithm::ControlAlgorithm()
{
    ...
    PreSI = { {VAS, 0.3},    {RF, 0.01},   {GLU, 0.25},   {HFL, 0.25}};// map pre stimulation added/subtracted to during stance initiation
    float prestance_hip = 0.05;
    PreSTANCE = { {VAS, 0.5},    {HFL, prestance_hip},   {GLU, prestance_hip},   {HAM, prestance_hip} };// map pre stimulation values for vas, hfl, ham, glu during stance
    // 
...
float ControlAlgorithm::MTU_excitation(Mtuname m_name,vector<bool> gait_state,char lead, char lat, float l_til, float f_norm, dVector com_Playervel)
{
    ...
    float gf = 0, glg = 0, glh = 0, gPDk = 0, gPDd = 0, gPDa = 0, gLead1 = 0, gLead2 = 0, gLead3 = 0, gP1 = 0, gP2 = 0, pre_s_si=0, pre_s_stance = 0;
    ...
    pre_s_si = GetPreStimSI(m_name);
    pre_s_stance = GetPreStimStance(m_name);

    float u = 0;
    float p = 0.01, q = 0.01;// initial constant excitation
    ...
    if (STANCE) {
        ...
        case GAS:
            u = p + uf_l[muscle].front();break;
        case VAS: {
            u = pre_s_stance + uf_l[muscle].front() + up_l[muscle].front();break;}
        case HAM: {
            if (uHIP_s_l[muscle].front() <  0)// torque on trunk rotates the body forward
                u = pre_s_stance + abs(uHIP_s_l[muscle].front());
            else
                u = pre_s_stance;
            break;}
        case GLU: {
    ...
    if (SWING) {
        ...
        case HFL: {
            string name = "ham_";
            name += lat;
            u = q + ul_l[muscle].front() - ul_l[name].front() + up_l[muscle].front();break;}// !!!!the minus ul leads to negative excitation!!!!
        //case VAS:
        //    u = q;break;
        //case RF:
        //    u = q;break;
    ...
    if (u < p)// security check: no negative values
        u = p;
    else if (u > 1) // security check: excitation max = 1. Suggested by Geyer, Appendix I first column.
        u = 1;
    return u;
}
...
float ControlAlgorithm::GetPreStimSI(Mtuname NAME)
{
    float g = 0;
    if (NAME == VAS || NAME == RF || NAME == GLU || NAME == HFL)
        g = PreSI.find(NAME)->second;
    else
        g = 0;
    return g;
}

float ControlAlgorithm::GetPreStimStance(Mtuname NAME)
{
    float g = 0;
    if (NAME == VAS || NAME == HAM || NAME == GLU || NAME == HFL)
        g = PreSTANCE.find(NAME)->second;
    else
        g = 0;
    return g;
}
...
void ControlAlgorithm::SetOptimizationParam(double* parameters)
{
    ...
    //Pre stimulation Stance Initiation 
    PreSI.find(VAS)->second = parameters[40];
    PreSI.find(RF)->second = parameters[41];
    PreSI.find(GLU)->second = parameters[42];
    PreSI.find(HFL)->second = parameters[43];

    // pre stimulation stance 
    PreSTANCE.find(VAS)->second = parameters[44];
    PreSTANCE.find(HAM)->second = parameters[45];//THE SAME
    PreSTANCE.find(GLU)->second = parameters[45];//THE SAME
    PreSTANCE.find(HFL)->second = parameters[45];//THE SAME

    // stance preparation 

    GPDk.find(HFL)->second = parameters[46];// list gain  PD controller spring
    GPDd.find(HFL)->second = parameters[47];// list gain PD controller damper
    GPDa.find(HFL)->second = parameters[48];//list gain PD controller ang in rad.

    GPDk.find(GLU)->second = parameters[49];
    GPDd.find(GLU)->second = parameters[50];
    GPDa.find(GLU)->second = parameters[51];

    GPDk.find(VAS)->second = parameters[52];
    GPDd.find(VAS)->second = parameters[53];
    GPDa.find(VAS)->second = parameters[54];
    ...
    // aditional
    GP1.find(HFL)->second = parameters[59];
    GP1.find(VAS)->second = parameters[60];
    GP2.find(VAS)->second = parameters[61];
