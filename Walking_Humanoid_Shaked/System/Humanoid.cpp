...
const char* dWalkerModel::getIndent(unsigned int numIndents)
{
...
const char* dWalkerModel::getIndentAlt(unsigned int numIndents)
{
    static const char* pINDENT = "                                        ";
    static const unsigned int LENGTH = strlen(pINDENT);
    unsigned int n = numIndents * 2;
...
int dWalkerModel::dump_attribs_to_stdout(TiXmlElement* pElement, std::vector<float> &vector, unsigned int indent)
{
...
int dWalkerModel::dump_attribs_to_stdout2(TiXmlElement* pElement, std::map<std::string, float>& l, unsigned int indent)
{
    if (!pElement) return 0;
...
void dWalkerModel::dump_to_stdout(TiXmlNode* pParent, unsigned int indent)
{
    if (!pParent) return;
...
void dWalkerModel::dump_to_stdout(const char* pFilename)
{
...
dWalkerModel::dWalkerModel(WindowMain* winctx, const char* const modelName, const dMatrix& location, int linkMaterilID, double* params, WBV wbv)
...

{
    ...
    dMatrix matrix;// default PITCH
    NewtonBodyGetMatrix(Geomfloor->GetBody(), &matrix[0][0]);
    if (wbv.direction.compare("roll")==0)
        matrix = matrix * dYawMatrix(90.0f * dDegreeToRad);// ROLL
    FloorAct = new dCustomHingeActuator(matrix, Geomfloor->GetBody());// sinusoidal 1 rotational dof WBV
    ...
    controller.SetOptimizationParam(params);
...
void dWalkerModel::CreateDescreteContactFootCollision(NewtonBody* const footBody) const
{
...
vector<dFloat> dWalkerModel::GetSagittalState(string body)
{
...
vector<dFloat> dWalkerModel::GetTrunkCoronalState()
{   
...
vector<dFloat> dWalkerModel::GetTrunkTransverseState()
{
    dMatrix mat = rigid_element.find("LPT")->second->GetMatrix();
...
vector<dFloat> dWalkerModel::GetFootCoronalState(string body)
{
    dMatrix mat = rigid_element.find(body)->second->GetMatrix();
...
vector<dFloat> dWalkerModel::GetRelativeSagittalState(string body1, string body2)
{
...
vector<dFloat> dWalkerModel::GetRelativeTransverseState(string body1, string body2)
{
    dMatrix mat = rigid_element.find(body2)->second->GetMatrix();
...
float dWalkerModel::ComputeAngleBWVectors(dVector const& dir, dVector const& ref, dVector const& crossref)
{
...
dVector dWalkerModel::ComputeWalkerCOM()
{
...
dVector dWalkerModel::ComputeWalkerCOMvelocity()
{
...
GeomNewton* dWalkerModel::Get_Floor()
{
    return Geomfloor;
}

map<std::string, GeomNewton*> dWalkerModel::Get_RigidElemetList()
{
    return rigid_element;
}

map<std::string, dCustomHinge*> dWalkerModel::Get_HingeJointList()
{
    return JHinge;
}
map<std::string, dCustomDoubleHinge*> dWalkerModel::Get_DoubleHingeJointList()
{
    return JDoubleHinge;
}

float dWalkerModel::GetLegLength()
{
    return l_Up_Leg + l_Low_Leg;
}


void dWalkerModel::AssemblyCreation()
{
...
void dWalkerModel::ArmCreation(string trunk, string uparm, string farm, string hand, string shoulder, string elbow, string wrist)
{
...
void dWalkerModel::LegCreation(string trunk, string thigh, string shank, string foot, string toes, string hip, string knee, string ankle, string toe_hinge)
{
...
void dWalkerModel::TrunkCreation()
{
    // Trunk creation
...
void dWalkerModel::HingeJoint(string jname, string body1, string body2, dVector pos, float minAng, float maxAng)
{
...
void dWalkerModel::ApplyTilting(float angle)
{
    FloorAct->SetTargetAngle(angle);
}

void dWalkerModel::DoubleHingeJoint(string jname, string body1, string body2, dVector pos, float minAng1, float maxAng1, float minAng2, float maxAng2)
{
    GeomNewton* b1 = rigid_element.find(body1)->second;
    GeomNewton* b2 = rigid_element.find(body2)->second;
    dMatrix matrix(dGetIdentityMatrix()); // define the pin
...
float dWalkerModel::GetHipLength()
{
    return l_Hip;
}

float dWalkerModel::GetModelMass()
{
    return Model_Mass;
}

void dWalkerModel::RigidBodyCreation(string body)
{
...
void dWalkerModel::DrawFrame(const dVector& posit, dFloat scale)
{
...
void dWalkerModel::AddActionReactionTorque(float Torque, dVector pin, GeomNewton* b1, GeomNewton* b2)
{
    if (b1 != NULL) {
...
WalkerManager::WalkerManager(WindowMain* winctx)
...
dWalkerModel* WalkerManager::CreateWalkerPlayer(const char* const modelName, const dMatrix& location, double* params, WBV wbv)
{
    dWalkerModel* const controller = new dWalkerModel(m_winManager, modelName, location, 0, params, wbv);
...
int WalkerManager::GetContactPoints(NewtonBody* const body, dVector* const points) const
{
    dVector point(0.0f);
    dVector normal(0.0f);
    int count = 0;
...
void WalkerManager::OnPreUpdate(dModelRootNode* const model, dFloat timestep, int threadID) const
//pour VS<VS2017 supprimer ,int threadID
{
    dFloat newTime = m_winManager->GetSimulationTime();
    dWalkerModel* Model = (dWalkerModel*)model;
    ...
    /// MUSCLE STUFF /////////////////////////////////////////////////////////////////////////////////////////////
    // find ankle position
    dVector com_Player = Model->ComputeWalkerCOM();
    dVector com_Player_vel = Model->ComputeWalkerCOMvelocity();
    ...
    if (com_Player.m_y > 0.6) {// guess value 
        ...
        GeomNewton* footR = RE_list.find("Foot_r")->second;
        GeomNewton* footL = RE_list.find("Foot_l")->second;
        com_ankle_r = Rel2AbsPoint(footR->GetBody(), &com_ankle_r.m_x);
        ankle = JDoubleHinge.find("Ankle_l")->second;
        mat = ankle->GetMatrix0();// error fix
        com_ankle_l = mat.m_posit;
        com_ankle_l = Rel2AbsPoint(footL->GetBody(), &com_ankle_l.m_x);

        // find contact between foot and ground.
        float count_contact = 0;
        dVector supportPolygon[32];
        int count = 0;
        count += GetContactPoints(footR->GetBody(), &supportPolygon[count]);
        //GeomNewton* BODY0 = RE_list.find("Toe_r")->second;
        ...
        count = 0;
        count += GetContactPoints(footL->GetBody(), &supportPolygon[count]);
        //GeomNewton* BODY1 = RE_list.find("Toe_l")->second;
            ...
            itr != m_winManager->aManager->vMuscleList.end(); itr++)
        {
            ...
            Mobj->SetNeuralDelay(1.f / 3000.f);
            ...
            Model->controller.UpdateMuscleReward(Ttemp.m_x* Ttemp.m_x + Ttemp.m_y* Ttemp.m_y);// update torque reward
            ...
            if (Model->write_txt)
            {
                ...
                {
                    if (lat == 'R')
                        Model->HFLr_T1.push_back(Ttemp.m_x);
                    else
                        Model->HFLl_T1.push_back(Ttemp.m_x);
                }
                case HAM:
                {
                    if (lat == 'R') {
                        Model->HAMr_T1.push_back(Ttemp.m_x);
                        Model->HAMr_T2.push_back(Ttemp.m_y);
                    }
                    else {
                        Model->HAMl_T1.push_back(Ttemp.m_x);
                        Model->HAMl_T2.push_back(Ttemp.m_y);
                    }
                }
                case GLU:
                {
                    if (lat == 'R')
                        Model->GLUr_T1.push_back(Ttemp.m_x);
                    else
                        Model->GLUl_T1.push_back(Ttemp.m_x);
                }
                case GAS:
                {
                    if (lat == 'R') {
                        Model->GASr_T1.push_back(Ttemp.m_x);
                        Model->GASr_T2.push_back(Ttemp.m_y);
                    }
                    else {
                        Model->GASl_T1.push_back(Ttemp.m_x);
                        Model->GASl_T2.push_back(Ttemp.m_y);
                    }
                }
                case VAS:
                {
                    if (lat == 'R')
                        Model->VASr_T1.push_back(Ttemp.m_x);
                    else
                        Model->VASl_T1.push_back(Ttemp.m_x);
                }
                case SOL:
                {
                    if (lat == 'R')
                        Model->SOLr_T1.push_back(Ttemp.m_x);
                    else
                        Model->SOLl_T1.push_back(Ttemp.m_x);
                }
                case TA:
                {
                    if (lat == 'R')
                        Model->TAr_T1.push_back(Ttemp.m_x);
                    else
                        Model->TAl_T1.push_back(Ttemp.m_x);
                }
                case RF:
                {
                    if (lat == 'R') {
                        Model->RFr_T1.push_back(Ttemp.m_x);
                        Model->RFr_T2.push_back(Ttemp.m_y);
                    }
                    else {
                        Model->RFl_T1.push_back(Ttemp.m_x);
                        Model->RFl_T2.push_back(Ttemp.m_y);
                    }
                }
                }
            }
        ...
        if (Model->write_txt)
        {
            ...
            dVector COM = Model->ComputeWalkerCOM();
            dVector COMvel = Model->ComputeWalkerCOMvelocity();

            dVector footr_c3d = footR->GetMatrix().m_posit;
            dVector footl_c3d = footL->GetMatrix().m_posit;

            Model->HipRAng.push_back(statehipr[0]);
            Model->HipLAng.push_back(statehipl[0]);
            Model->KneeRAng.push_back(statekneer[0]);
            Model->KneeLAng.push_back(statekneel[0]);
            Model->AnkleRAng.push_back(stateankler[0]);
            Model->AnkleLAng.push_back(stateanklel[0]);

            Model->COMPos_X.push_back(COM.m_x);
            Model->COMPos_Y.push_back(COM.m_y);
            Model->COMPos_Z.push_back(COM.m_z);

            Model->COMVel_X.push_back(COMvel.m_x);
            Model->COMVel_Y.push_back(COMvel.m_y);
            Model->COMVel_Z.push_back(COMvel.m_z);

            Model->FootRPos_X.push_back(footr_c3d.m_x);
            Model->FootRPos_Y.push_back(footr_c3d.m_y);
            Model->FootRPos_Z.push_back(footr_c3d.m_z);

            Model->FootLPos_X.push_back(footl_c3d.m_x);
            Model->FootLPos_Y.push_back(footl_c3d.m_y);
            Model->FootLPos_Z.push_back(footl_c3d.m_z);

            Model->StanceR.push_back(state_foot_r[0]);
            Model->SwingR.push_back(state_foot_r[1]);
            Model->SPR.push_back(state_foot_r[2]);
            Model->SIR.push_back(state_foot_r[3]);
 
            Model->StanceL.push_back(state_foot_l[0]);
            Model->SwingL.push_back(state_foot_l[1]);
            Model->SPL.push_back(state_foot_l[2]);
            Model->SIL.push_back(state_foot_l[3]);
            ...
            //    "  " << state_foot_l[0] << "  " << state_foot_l[1] << "  " << state_foot_l[2] << "  " << state_foot_l[3] << "  " << 
            //    footr_c3d.m_x << "  " << footr_c3d.m_y << "  " << footr_c3d.m_z << "  " <<
            //    footl_c3d.m_x << "  " << footl_c3d.m_y << "  " << footl_c3d.m_z << "  " <<
            //    COM.m_x << "  " << COM.m_y << "  " << COM.m_z << "  " << COMvel.m_x << "  " << COMvel.m_y << "  " << COMvel.m_z << endl;
        }

        Model->time_v.push_back(newTime);
    ...
    dVector COM_vel = Model->ComputeWalkerCOMvelocity();
    Model->controller.UpdateTaskReward(com_Player.m_y, COM_vel, Strunk[0], Ctrunk[0]);
    
    // ADD WBV

    float amp = m_winManager->GetWVBAmp();
    if (newTime <= 2.0)
        amp = amp * (newTime)/2.0;
    Model->ApplyTilting( amp* dDegreeToRad * sin(m_winManager->GetWVBFreq() * 2 * M_PI * (newTime)));
...
void WalkerManager::OnPostUpdate(dModelRootNode* const model, dFloat timestep, int threadID) const
//pour VS<VS2017 supprimer ,int threadID
...
void WalkerManager::OnUpdateTransform(const dModelNode* const bone, const dMatrix& localMatrix) const
{
...
void WalkerManager::OnDebug(dModelRootNode* const model, dCustomJoint::dDebugDisplay* const debugContext)
{
