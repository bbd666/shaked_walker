...

{
	...
	Scale = 2.0f; // Scale factor of dummy

	// LENGTH DATA
	l_Up_Leg	= Scale * 0.4222f;
	l_Low_Leg	= Scale * 0.434f;
	l_plantar	= Scale * 0.183f; // taken from Chadefaux et al
	l_Spine		= Scale * 0.3862f; //assumed to be upper part of trunk
	l_Up_Arm	= Scale * 0.28f;
	l_Low_Arm	= Scale * 0.27f;
	l_Hand		= Scale * 0.0862f;
	l_Head		= Scale * 0.2033f;
	l_Hip		= Scale * 0.15f; //half length of hip
	l_Sacrum	= Scale * 0.1457f; //assumed to be Lower part of trunk
	w_foot		= Scale * l_plantar/2.0f; //
	l_toe		= Scale * 0.06f; // taken from Chadefaux et al
	h_foot		= Scale * 0.1f;
	l_Neck		= Scale * 0.0396f;
	l_Clav		= Scale * 0.225f; //half length of shoulder width
	r_bones		= Scale * 0.07f;
	r_Pad		= w_foot/4.0f;
	h_sphere	= h_foot / 2;

	std::vector<float> l = {Scale * l_Head, Scale * l_Spine, Scale * l_Up_Arm, Scale * l_Low_Arm, Scale * l_Hand, Scale * l_Up_Leg, Scale * l_Low_Leg, Scale * l_plantar };
	
	// MASSE DATA
	float tot_w = Scale * 73.0f; //private variable?
	std::vector<float> masses  = { 0.0694f * tot_w, 0.4346f * tot_w, 0.0271f * tot_w, 0.0162f * tot_w, 0.0061f * tot_w, 0.1416f * tot_w, 0.0433f * tot_w, 0.0137f * tot_w};
	
	//INERTIA DATA
	//	     				 {		0 head,			1 trunk,		 2 uparm,		3 forearm,	     4 hand,			5 thigh,		 6 shank,		  7 foot } table 4 de leva technical notes
	std::vector<float> Ixx = { masses[0] * 0.362f, masses[1] * 0.372f, masses[2] * 0.285f, masses[3] * 0.276f, masses[4] * 0.628f, masses[5] * 0.329f, masses[6] * 0.255f, masses[7] * 0.257f }; //radii of gyration Sagittal (X) %
	std::vector<float> Iyy = { masses[0] * 0.376f, masses[1] * 0.347f, masses[2] * 0.269f, masses[3] * 0.265f, masses[4] * 0.513f, masses[5] * 0.329f, masses[6] * 0.249f, masses[7] * 0.245f }; //radii of gyration Transverse (Y) %
	std::vector<float> Izz = { masses[0] * 0.312f, masses[1] * 0.191f, masses[2] * 0.158f, masses[3] * 0.121f, masses[4] * 0.401f, masses[5] * 0.149f, masses[6] * 0.103f, masses[7] * 0.124f }; //radii of gyration Longitudinal (Z) %
	...
	vector<float> DeltaCM(comY.size(), 0);

	for (int ii = 0; ii < DeltaCM.size(); ii++)
		DeltaCM[ii] = (0.5f - comY[ii]) * l[ii];

	glm::vec3 _Pos(glm::vec3(0.0f, 3.0f, 0.f)); // INITIAL POSITION  OF tHE DUMMY IN THE SCENE

	float hip_angle = -30.0f;
	float clav_angle = 0.0f;
