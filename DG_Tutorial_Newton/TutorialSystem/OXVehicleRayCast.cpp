...

{
	...
	if (v_scale.size() != 2 || v_lengths.size() != 15 || v_weight.size() != 1 || v_masses.size() != 8 || v_ixx.size() != 8 || v_iyy.size() != 8 || v_izz.size() != 8 || v_com.size() != 8 || v_angles.size() != 2)
	...
	// LENGTH DATA
	l_Up_Leg	= scale_length * v_lengths[0];
	l_Low_Leg	= scale_length * v_lengths[1];
	l_plantar	= scale_length * v_lengths[2];		// taken from Chadefaux et al
	l_Spine		= scale_length * v_lengths[3];		//assumed to be upper part of trunk
	l_Up_Arm	= scale_length * v_lengths[4];
	l_Low_Arm	= scale_length * v_lengths[5];
	l_Hand		= scale_length * v_lengths[6];
	l_Head		= scale_length * v_lengths[7];
	l_Hip		= scale_length * v_lengths[8];		//half length of hip
	l_Sacrum	= scale_length * v_lengths[9];		//assumed to be Lower part of trunk
	w_foot		= l_plantar/2.0f;	
	l_toe		= scale_length * v_lengths[10];	//taken from Chadefaux et al
	h_foot		= scale_length * v_lengths[11];
	l_Neck		= scale_length * v_lengths[12];
	l_Clav		= scale_length * v_lengths[13];	//half length of shoulder width
	r_bones		= v_lengths[14];
	r_Pad		= w_foot/4.0f;
	h_sphere	= h_foot / 2;
	...
	// MASSE DATA
	float tot_w = scale_mass * v_weight[0]; //private variable?
	...
	Pad1_L->SetPosition(   w_foot / 2, -  h_sphere, - (l_plantar / 2 - r_Pad));
	Pad1_L->InitNewton(atSphere, r_Pad, r_Pad, r_Pad, 10.0f  );
	NewtonBodySetTransformCallback(Pad1_L->GetBody(), NULL);
	Pad1_L_Node = new dModelNode(Pad1_L->GetBody(), dGetIdentityMatrix(), Plantar_LNode);
	...
	Pad2_L->SetPosition( -  w_foot / 2, -  h_sphere, -  (l_plantar / 2 - r_Pad));
	Pad2_L->InitNewton(atSphere, r_Pad, r_Pad, r_Pad, 10.0f  );
	NewtonBodySetTransformCallback(Pad2_L->GetBody(), NULL);
	Pad2_L_Node = new dModelNode(Pad2_L->GetBody(), dGetIdentityMatrix(), Plantar_LNode);
	...
	Pad1_R->SetPosition(  w_foot / 2, -  h_sphere, -(l_plantar / 2 - r_Pad));
	Pad1_R->InitNewton(atSphere, r_Pad, r_Pad, r_Pad, 10.0f  );
	NewtonBodySetTransformCallback(Pad1_R->GetBody(), NULL);
	Pad1_R_Node = new dModelNode(Pad1_R->GetBody(), dGetIdentityMatrix(), Plantar_RNode);
	...
	Pad2_R->SetPosition(-  w_foot / 2, -  h_sphere, -(l_plantar / 2 - r_Pad));
	Pad2_R->InitNewton(atSphere, r_Pad, r_Pad, r_Pad, 10.0f  );
	NewtonBodySetTransformCallback(Pad2_R->GetBody(), NULL);
	Pad2_R_Node = new dModelNode(Pad2_R->GetBody(), dGetIdentityMatrix(), Plantar_RNode);
	...
	Heel_L_PinMatrix.m_posit = dVector(0, -  h_sphere,   (l_plantar / 2), 1.0f);
	...
	Pad1_L_PinMatrix.m_posit = dVector(  w_foot / 2, -  h_sphere, -  (l_plantar / 4), 1.0f);
	...
	Pad2_L_PinMatrix.m_posit = dVector(-  w_foot / 2, -  h_sphere, -  (l_plantar / 4), 1.0f);
	...
	Heel_R_PinMatrix.m_posit = dVector(0, -  h_sphere,   (l_plantar / 2), 1.0f);
	...
	Pad1_R_PinMatrix.m_posit = dVector(  w_foot / 2, -  h_sphere, -  (l_plantar / 4), 1.0f);
	...
	Pad2_R_PinMatrix.m_posit = dVector(-  w_foot / 2, -  h_sphere, -  (l_plantar / 4), 1.0f);
	...
	Neck_PinMatrix.m_posit = dVector(Neck->GetPosition().m_x, Neck->GetPosition().m_y -   l_Neck / 2 -    r_bones, Neck->GetPosition().m_z, 1.0f);
	...
	Sternum_PinMatrix.m_posit = dVector(Spine->GetPosition().m_x +   r_bones, Spine->GetPosition().m_y+   l_Spine / 2, Spine->GetPosition().m_z, 1.0f);
	...
	Sacrum_PinMatrix.m_posit = dVector(_Pos.x +   r_bones, _Pos.y -   l_Sacrum / 2, _Pos.z, 1.0f);
	...
	Toe_LPinMatrix.m_posit = dVector(Plantar_L->GetPosition().m_x, Plantar_L->GetPosition().m_y, Plantar_L->GetPosition().m_z-   l_plantar/2, 1.0f);
	...
	Toe_RPinMatrix.m_posit = dVector(Plantar_R->GetPosition().m_x, Plantar_R->GetPosition().m_y, Plantar_R->GetPosition().m_z -   l_plantar/2 ,1.0f);
