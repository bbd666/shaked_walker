...
#endif
{
	int operation = 2; //"To run simulation enter 1, to run optimization enter 2:", else nothing.
	// take inputs
	#if defined(NDEBUG)
	...
	if (operation == 2) // run optimization
	{
...
float CallNewton(double* params, int operation, int iteration_max, int current_iteration, int popsize)
{
	...
	DGVehicleRCManager* aWalkerManager = new DGVehicleRCManager(aManager);
	dMatrix matrix(dGetIdentityMatrix());
	dRaycastVHModel* const Model = aWalkerManager->CreateWalkerPlayer("WALKER", matrix);
	dResetTimer(); // for dGetElapsedSeconds()
	aManager->SetSimulationTime(0.0);
	aManager->SetMaxSimulationTime(5.0);
	Model->controller.SetOptimizationParam(params);
	float Time = 0;
	while (Time < 5)//&& Time < 5.0f stop simulation if window is closed or simulation time is 10 s
	{
		...
		Time = aManager->GetSimulationTime(); 
	}
	float cost = 0, mass = 0;
	vector<float> reward;
	...
	float wm = 100;// weight for Muscles torques
	float wpd = 1; // weight for PD torques
	float T = 3000 * aManager->GetMaxSimulationTime(); // 3000 Hz * simulation time (5s) 
	cost = reward[0] + we / T * (wm * reward[1] + wpd * reward[2]);
